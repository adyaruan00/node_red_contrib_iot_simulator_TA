module.exports = function(RED) {
                ...
                if ( config.allinone ) { 
                    ...
                    for (var i=0; i< config.options.length; i++) {
                        ...
                        if ( !isNaN(parseFloat(option.value)) && isFinite(option.value) ) {  
                            if ( range != 0  && option.value <= 475 && option.range <=475) {  
                    ...
                    for (var i=0; i< config.options.length; i++) {
                        ...
                        if ( !isNaN(parseFloat(option.value)) && isFinite(option.value) )  {            
                            if ( range != 0 ) {  
                                ...
                                msg.payload[ 'latitude'] = option.latitude;
                                msg.payload[ 'longitude'] = option.longitude;
                                msg.payload['Value']=Math.floor(Math.random() * (maxValue - minValue + 1) ) + minValue;  
                            } else 
                           msg.payload['Sensor']=option.label ;
                            msg.payload[ 'ID'] = option.idkey;
                            msg.payload[ 'latitude'] = option.latitude;
    ...
    RED.nodes.registerType("sensor-kelembapan",IoTSimulatorNode);
}
